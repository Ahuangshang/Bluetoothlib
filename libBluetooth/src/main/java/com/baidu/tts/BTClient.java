...
import com.baidu.tts.service.BluetoothLeService;
...
public class BTClient {
    ...
    public static void init(Context context){
        //需要先启动BluetoothManagerService
        Intent intent = new Intent(context, BluetoothManagerService.class);
    ...
    private static void getCRC(byte[] data, int Len) {
        int i, j;
    ...
    private static boolean CheckCRC(byte[] data, int len) {
    ...
    public static int GetData() {
        ...
        while ((System.currentTimeMillis() - time1) < 2000) {
            ...
            int recvLen = BluetoothManagerService.RecvString.length() / 2;
            if (recvLen > 0) {
                byte[] buffer = new byte[recvLen];
                buffer = hexStringToBytes(BluetoothManagerService.RecvString);
                memcpy(buffer, 0, RecvBuff, 0, recvLen);
                RecvLength = recvLen;
                int activelen = buffer[0] + 1;
                if (CheckCRC(RecvBuff, activelen)) {
                    Log.d("read data:", BluetoothManagerService.RecvString);
                    CmdIng = false;
                    return 0;
                }
            }
    ...
    public static int GetReaderInfo(byte[] Version, byte[] Power, byte[] Fre) {
        ...
        Msg[4] = (byte) 0x95;
        CmdTime = 500;
        BluetoothManagerService.target_chara.setValue(Msg);
        ArrayClear(RecvBuff, 5000);
        RecvLength = 0;
        BluetoothManagerService.RecvString = "";
        mBluetoothLeService.writeCharacteristic(BluetoothManagerService.target_chara);
        if (GetData() == 0) {
            ComAddr = RecvBuff[1];
    ...
    public static int SetPower(byte Pwr) {
        if (BluetoothManagerService.target_chara != null) {
            byte[] Msg = new byte[6];
            Msg[0] = 0x05;
    ...
    public static int Inventory_G2(byte QValue, byte Session, byte AdrTID, byte LenTID,
        ...
        if (BluetoothManagerService.target_chara == null) {
            Log.e("TAG", "Inventory_G2: target_chara为" + null);
            return -1;
        }
        BluetoothManagerService.target_chara.setValue(Msg);
        ArrayClear(RecvBuff, 300);
    ...
    public static int ReadData_G2(byte Enum, byte[] EPC, byte Mem, byte WordAddr, byte Num, byte[] Psd, byte[] Data) {
        ...
        while (SendFlag) {
            if ((Len - 20) > 0) {
                ...
                Len = Len - 20;
                BluetoothManagerService.target_chara.setValue(data);
                mBluetoothLeService.writeCharacteristic(BluetoothManagerService.target_chara);
